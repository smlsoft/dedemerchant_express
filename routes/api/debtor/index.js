...
const queueGenDebtorReport = new Queue("dedemerchantDebtorReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenDebtorReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadDebtorPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenDebtorReport.on("completed", (job, result) => {
  logger.info(`Job Debtor completed with result ${result}`);
});

queueGenDebtorReport.on("failed", (job, err) => {
  logger.error(`Job Debtor failed with error `, err.message);
});

router.get("/genPDFDebtor", async (req, res) => {
  try {
    ...
    let fileName = `debtor-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenDebtorReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-debtor/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-debtor/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenDebtorReport.getJob(jobId);
...
router.get("/download-debtor/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
