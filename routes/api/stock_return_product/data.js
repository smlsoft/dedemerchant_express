...
const dataresult = async (token, search, fromdate, todate) => {
  ...
  try {
    ...
    const data = db.collection("transactionStockReturnProduct");
...
const genPDF = async (body,dataprofile) => {
  var docDefinition = {
    content: [
      {
        text: "รายงานรับคืนสินค้าจากการเบิก",
        style: "header",
        alignment: "center",
      },
      {
...
const genDownLoadReturnProductPDF = async (token, search, fromdate, todate, fileName) => {
...
module.exports = { dataresult, genPDF, pdfPreview, pdfDownload, sendEmail , genDownLoadReturnProductPDF};
