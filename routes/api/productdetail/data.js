...
const dataresult = async (where,shopid) => {
  const query = `select shopid ,code,arrayFirst(x -> x != '', p.names) as itemname ,unitcost ,unitstandard , producttype ,taxtype 
  from dede001.product p where p.shopid='${shopid}' ${where} order by p.shopid,p.code`;
