...
router.get("/", async (req, res) => {
  try {
    ...
    if (req.query.pricemember != undefined && req.query.pricemember != "") {
      where +=` and pricemember in (${req.query.pricemember}) `
    }
...
const _whereLike = async (strKeyWord) => {
  ...
  if (strKeyWord.trim().length > 0) {
    var __fieldList = ["p.barcode", "itemname", "p.iccode", "p.unitcode"];
