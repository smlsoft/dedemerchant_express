...
const queueGenMovementReport = new Queue("dedemerchantMovementInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenMovementReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadMovementPDF(
...
queueGenMovementReport.on("completed", (job, result) => {
  logger.info(`Job Movement completed with result ${result}`);
});

queueGenMovementReport.on("failed", (job, err) => {
  logger.error(`Job Movement failed with error `, err.message);
});

router.get("/genPDFMovement", async (req, res) => {
  try {
    ...
    let fileName = `movement-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenMovementReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-movement/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-movement/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenMovementReport.getJob(jobId);
...
router.get("/download-movement/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
