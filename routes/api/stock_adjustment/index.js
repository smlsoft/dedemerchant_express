...
const queueGenStockAdjustmentReport = new Queue("dedemerchantStockAdjustmentInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenStockAdjustmentReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadStockAdjustmentPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenStockAdjustmentReport.on("completed", (job, result) => {
  logger.info(`Job Stock Adjustment completed with result ${result}`);
});

queueGenStockAdjustmentReport.on("failed", (job, err) => {
  logger.error(`Job Stock Adjustment failed with error `, err.message);
});

router.get("/genPDFStockAdjustment", async (req, res) => {
  try {
    ...
    let fileName = `stock-adjustment-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenStockAdjustmentReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-stock-adjustment/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-stock-adjustment/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenStockAdjustmentReport.getJob(jobId);
...
router.get("/download-stock-adjustment/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
