...
const queueGenVatPurchaseReport = new Queue("dedemerchantVatPurchaseInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenVatPurchaseReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadVatPurchasePDF(
    payload.data.shopid,
    payload.data.year,
    payload.data.month,
...
queueGenVatPurchaseReport.on("completed", (job, result) => {
  logger.info(`Job Vat Purchase completed with result ${result}`);
});

queueGenVatPurchaseReport.on("failed", (job, err) => {
  logger.error(`Job Vat Purchase failed with error `, err.message);
});

router.get("/genPDFVatPurchase", async (req, res) => {
  ...
  try {
    ...
    let fileName = `vat-purchase-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenVatPurchaseReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-vat-purchase/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-vat-purchase/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenVatPurchaseReport.getJob(jobId);
...
router.get("/download-vat-purchase/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
