...
const queueGenSaleInvReturnReport = new Queue("dedemerchantSaleInvReturnInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenSaleInvReturnReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadSaleInvReturnPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenSaleInvReturnReport.on("completed", (job, result) => {
  logger.info(`Job SaleInvReturn completed with result ${result}`);
});

queueGenSaleInvReturnReport.on("failed", (job, err) => {
  logger.error(`Job SaleInvReturn failed with error `, err.message);
});

router.get("/genPDFSaleInvReturn", async (req, res) => {
  try {
    ...
    let fileName = `saleinv-return-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenSaleInvReturnReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-saleinv-return/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-saleinv-return/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenSaleInvReturnReport.getJob(jobId);
...
router.get("/download-saleinv-return/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
