...
const queueGenStockCardReport = new Queue("dedemerchantStockCardInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenStockCardReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadStockCardPDF(
...
queueGenStockCardReport.on("completed", (job, result) => {
  logger.info(`Job StockCard completed with result ${result}`);
});

queueGenStockCardReport.on("failed", (job, err) => {
  logger.error(`Job StockCard failed with error `, err.message);
});

router.get("/genPDFStockCard", async (req, res) => {
  try {
    ...
    let fileName = `stockcard-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenStockCardReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-stockcard/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-stockcard/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenStockCardReport.getJob(jobId);
...
router.get("/download-stockcard/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
