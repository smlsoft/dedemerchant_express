...
const queueGenProductReport = new Queue("dedemerchantProductReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenProductReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadProductPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenProductReport.on("completed", (job, result) => {
  logger.info(`Job Product completed with result ${result}`);
});

queueGenProductReport.on("failed", (job, err) => {
  logger.error(`Job Product failed with error `, err.message);
});

router.get("/genPDFProduct", async (req, res) => {
  try {
    ...
    let fileName = `product-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenProductReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-product/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-product/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenProductReport.getJob(jobId);
...
router.get("/download-product/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
