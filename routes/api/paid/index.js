...
const queueGenPaidReport = new Queue("dedemerchantPaidInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenPaidReport.process(async (payload) => {
...
queueGenPaidReport.on("completed", (job, result) => {
  logger.info(`Job Paid completed with result ${result}`);
});

queueGenPaidReport.on("failed", (job, err) => {
  logger.error(`Job Paid failed with error `, err.message);
});

router.get("/genPDFPaid", async (req, res) => {
  try {
    ...
    let fileName = `paid-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenPaidReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-paid/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-paid/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenPaidReport.getJob(jobId);
...
router.get("/download-paid/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
