...
const queueGenPickupReport = new Queue("dedemerchantPickupInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenPickupReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadPickupPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenPickupReport.on("completed", (job, result) => {
  logger.info(`Job Pickup completed with result ${result}`);
});

queueGenPickupReport.on("failed", (job, err) => {
  logger.error(`Job Pickup failed with error `, err.message);
});

router.get("/genPDFPickup", async (req, res) => {
  try {
    ...
    let fileName = `pickup-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenPickupReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-pickup/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-pickup/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenPickupReport.getJob(jobId);
...
router.get("/download-pickup/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
