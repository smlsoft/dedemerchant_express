...
const queueGenPurchaseReturnReport = new Queue("dedemerchantPurchaseReturnInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenPurchaseReturnReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadPurchaseReturnPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenPurchaseReturnReport.on("completed", (job, result) => {
  logger.info(`Job PurchaseReturn completed with result ${result}`);
});

queueGenPurchaseReturnReport.on("failed", (job, err) => {
  logger.error(`Job PurchaseReturn failed with error `, err.message);
});

router.get("/genPDFPurchaseReturn", async (req, res) => {
  try {
    ...
    let fileName = `purchase-return-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenPurchaseReturnReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-purchase-return/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-purchase-return/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenPurchaseReturnReport.getJob(jobId);
...
router.get("/download-purchase-return/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
