...
const queueGenReceiveReport = new Queue("dedemerchantReceiveInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenReceiveReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadReceivePDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenReceiveReport.on("completed", (job, result) => {
  logger.info(`Job Receive completed with result ${result}`);
});

queueGenReceiveReport.on("failed", (job, err) => {
  logger.error(`Job Receive failed with error `, err.message);
});

router.get("/genPDFReceive", async (req, res) => {
  try {
    ...
    let fileName = `receive-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
...
router.get("/check-receive/:jobId/:filename", async (req, res) => {
  ...
  const job = await queueGenReceiveReport.getJob(jobId);
