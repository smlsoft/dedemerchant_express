...
const queueGenPayReport = new Queue("dedemerchantPayInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenPayReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadPayPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenPayReport.on("completed", (job, result) => {
  logger.info(`Job Pay completed with result ${result}`);
});

queueGenPayReport.on("failed", (job, err) => {
  logger.error(`Job Pay failed with error `, err.message);
});

router.get("/genPDFPay", async (req, res) => {
  try {
    ...
    let fileName = `pay-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenPayReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-pay/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-pay/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenPayReport.getJob(jobId);
...
router.get("/download-pay/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
