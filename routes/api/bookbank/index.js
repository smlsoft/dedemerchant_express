...
const queueGenBookBankReport = new Queue("dedemerchantBookBankReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenBookBankReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadBookBankPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenBookBankReport.on("completed", (job, result) => {
  logger.info(`Job BookBank completed with result ${result}`);
});

queueGenBookBankReport.on("failed", (job, err) => {
  logger.error(`Job BookBank failed with error `, err.message);
});

router.get("/genPDFBookBank", async (req, res) => {
  try {
    ...
    let fileName = `bookbank-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenBookBankReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-bookbank/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-bookbank/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenBookBankReport.getJob(jobId);
...
router.get("/download-bookbank/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
