...
const queueGenGetPayReport = new Queue("dedemerchantGetPayInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenGetPayReport.process(async (payload) => {
  logger.info("on process");

  data.genDownLoadGetPayPDF(
    payload.data.shopid,
    payload.data.search,
...
queueGenGetPayReport.on("completed", (job, result) => {
  logger.info(`Job Get Pay completed with result ${result}`);
});

queueGenGetPayReport.on("failed", (job, err) => {
  logger.error(`Job Get Pay failed with error `, err.message);
});

router.get("/genPDFGetPay", async (req, res) => {
  try {
    ...
    let fileName = `get-pay-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenGetPayReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-get-pay/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-get-pay/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenGetPayReport.getJob(jobId);
...
router.get("/download-get-pay/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
...
router.get("/pdfview", async (req, res) => {
  var result = await globalservice.getUserShop(req.query.token);
  if (!result) {
    res.status(500).json({ success: false, msg: "Invalid shop" });
    return;
  }
