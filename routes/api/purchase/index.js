...
const queueGenPurchaseReport = new Queue("dedemerchantPurchaseInvReport", process.env.REDIS_CACHE_URI + "?tls=" + process.env.REDIS_CACHE_TLS_ENABLE);

queueGenPurchaseReport.process(async (payload) => {
...
queueGenPurchaseReport.on("completed", (job, result) => {
  logger.info(`Job Purchase completed with result ${result}`);
});

queueGenPurchaseReport.on("failed", (job, err) => {
  logger.error(`Job Purchase failed with error `, err.message);
});

router.get("/genPDFPurchase", async (req, res) => {
  try {
    ...
    let fileName = `purchase-${result.data.name}-${utils.formateDateTime(Date.now())}.pdf`;
    ...
    queueGenPurchaseReport
      ...
      .then((job) => {
        ...
        res.status(200).json({
          ...
          data: { fileName: fileName, jobId: job.id, downloadLink: `${protocol}://${host}${desiredPath}download-purchase/${job.id}/${fileName}` },
        });
      })
      .catch((err) => {
...
router.get("/check-purchase/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
  const job = await queueGenPurchaseReport.getJob(jobId);
...
router.get("/download-purchase/:jobId/:filename", async (req, res) => {
  const jobId = req.params.jobId;
  const filename = req.params.filename;
